<template>
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center gap-2 text-sm text-surface-600 dark:text-surface-400">
      ...
      <span class="text-surface-500 dark:text-surface-300">優惠管理</span>
      <span>></span>
      <span class="text-surface-900 dark:text-surface-0 font-medium">優惠管理 (包網版)</span>
    </div>
         ...
         <TabView v-model:activeIndex="activeTab">
            ...
            <TabPanel header="優惠活動列表" value="activities">
                <div class="space-y-4">
                    ...
                    <DataTable :value="activities" stripedRows class="p-datatable-sm" :pt="{ table: { class: 'min-w-full' }, tbody: { class: 'text-surface-700 dark:text-surface-300' } }">
                        <Column field="name" header="活動名稱" style="min-width: 150px">
                         ...
                         <Column field="turnoverMulti" header="流水倍數" style="min-width: 100px">
                             <template #body="slotProps">
                                <span class="font-mono text-surface-900 dark:text-white">{{ slotProps.data.turnoverMulti }}x</span>
                            </template>
                        </Column>
            ...
            <TabPanel header="自動審核列隊" value="audit">
                 <div class="space-y-4">
                    <div class="flex gap-2 mb-4 bg-surface-100 dark:bg-surface-900 p-3 rounded items-center">
                        <span class="text-surface-500 dark:text-surface-400 text-sm">已選取: {{ selectedAudits.length }} 筆</span>
                    ...
                    <DataTable v-model:selection="selectedAudits" :value="auditQueue" dataKey="id" stripedRows class="p-datatable-sm" :pt="{ table: { class: 'min-w-full' }, tbody: { class: 'text-surface-700 dark:text-surface-300' } }">
                         <Column selectionMode="multiple" headerStyle="width: 3rem"></Column>
                         <Column field="memberAccount" header="會員帳號" style="min-width: 120px">
                             <template #body="slotProps">
                                <span class="text-surface-900 dark:text-white">{{ slotProps.data.memberAccount }}</span>
                            </template>
                        </Column>
                         ...
                         <Column header="流水達成狀況 (目前/目標)" style="min-width: 200px">
                             <template #body="slotProps">
                                <div class="flex items-center gap-2 font-mono">
                                    <span :class="slotProps.data.currentTurnover >= slotProps.data.targetTurnover ? 'text-green-500 dark:text-green-400' : 'text-yellow-500 dark:text-yellow-400'">
                                        {{ formatCurrency(slotProps.data.currentTurnover) }}
                                    </span>
                                    <span class="text-surface-500">/</span>
                                    <span class="text-surface-500 dark:text-surface-300">{{ formatCurrency(slotProps.data.targetTurnover) }}</span>
                                </div>
    ...
    <Dialog v-model:visible="activityDialogVisible" modal :header="currentActivity.id ? '編輯優惠活動' : '新增優惠活動'" :style="{ width: '500px' }">
        <div class="space-y-4 p-2">
             ...
             <div class="grid grid-cols-2 gap-4">
                 <div class="flex flex-col gap-2">
                    <label class="text-surface-500 dark:text-surface-300 text-sm">流水倍數 (x)</label>
                    <div style="width: 100px">
                        <InputNumber v-model="currentActivity.turnoverMulti" :min="1" :max="50" class="w-full" inputClass="text-center" />
                    </div>
                </div>
                <div class="flex flex-col gap-2">
                    <label class="text-surface-500 dark:text-surface-300 text-sm">活動狀態</label>
                    <Dropdown v-model="currentActivity.status" :options="['啟用', '停用']" class="w-full" />
                </div>
             </div>

             <div class="bg-surface-100 dark:bg-surface-900 p-3 rounded space-y-3 border border-surface-200 dark:border-surface-700">
                <div class="text-sm text-surface-900 dark:text-white font-medium mb-2">出資分攤設定 (總和 100%)</div>
...
/* Removed hardcoded input styles to allow theme defaults or global overrides */
</style>
