<template>
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center gap-2 text-sm text-surface-400">
      ...
      <span>></span>
      <span class="text-white font-medium">點數與獎勵</span>
    </div>
      ...
      <template #content>
        <div class="flex items-end justify-between gap-4">
          <div class="flex flex-col gap-1">
            <label class="text-surface-300 text-sm font-medium">快速搜尋會員</label>
    ...
    <Card class="bg-surface-800/50 border border-surface-700">
      ...
      <template #content>
         <DataTable :value="recentAdjustments" stripedRows class="p-datatable-sm" :pt="{ table: { class: 'min-w-full' }, tbody: { class: 'text-surface-300' } }">
            ...
            <Column field="operator" header="操作員" style="min-width: 100px">
                 <template #body="slotProps">
                    <div class="flex items-center gap-1">
                        <i class="pi pi-user text-xs text-blue-400"></i>
            ...
            <Column field="amount" header="異動金額" style="min-width: 120px">
                 <template #body="slotProps">
                     <span class="font-mono font-bold" :class="slotProps.data.amount >= 0 ? 'text-green-400' : 'text-red-400'">
    ...
    <Card class="bg-surface-800/50 border border-surface-700">
      ...
      <template #content>
         <DataTable :value="bulkTasks" stripedRows class="p-datatable-sm" :pt="{ table: { class: 'min-w-full' }, tbody: { class: 'text-surface-300' } }">
            <Column field="taskName" header="任務名稱" style="min-width: 200px">
                 <template #body="slotProps">
                    <span class="text-white font-medium">{{ slotProps.data.taskName }}</span>
                </template>
            </Column>
            ...
            <Column field="createTime" header="建立時間" style="min-width: 160px">
                 <template #body="slotProps">
                    <span class="text-sm text-surface-400">{{ slotProps.data.createTime }}</span>
                </template>
            </Column>
         </DataTable>
      </template>
    ...
    <Dialog v-model:visible="adjustmentDialogVisible" modal header="新增人工存提申請" :style="{ width: '500px' }">
        <div class="flex flex-col gap-4">
            ...
            <div v-if="memberBalance !== null" class="bg-blue-500/10 border border-blue-500/30 rounded p-3">
                <div class="text-sm text-surface-400">當前餘額</div>
                <div class="text-2xl font-bold text-blue-400 font-mono">{{ formatCurrency(memberBalance) }}</div>
            </div>

            <div class="flex flex-col gap-1">
                <label class="text-surface-300 text-sm font-medium">異動金額</label>
                ...
                <small class="text-surface-500">提示：正數為增點，負數為扣點</small>
            </div>

            <div class="flex flex-col gap-1">
                <label class="text-surface-300 text-sm font-medium">存提類型</label>
            ...
            <div class="flex flex-col gap-1">
                <label class="text-surface-300 text-sm font-medium">操作原因</label>
            ...
            <div v-if="Math.abs(adjustmentForm.amount || 0) > 10000" class="bg-yellow-500/10 border border-yellow-500/30 rounded p-3 flex items-start gap-2">
                <i class="pi pi-exclamation-triangle text-yellow-400 mt-1"></i>
    ...
    <Dialog v-model:visible="bulkTaskDialogVisible" modal header="建立批量發放任務" :style="{ width: '500px' }">
        <div class="flex flex-col gap-4">
            <div class="flex flex-col gap-1">
                <label class="text-surface-300 text-sm font-medium">任務名稱</label>
                <InputText v-model="bulkTaskForm.taskName" placeholder="例如：VIP 週年慶獎勵" />
            </div>

            <div class="flex flex-col gap-1">
                <label class="text-surface-300 text-sm font-medium">目標對象</label>
            ...
            <div class="flex flex-col gap-1">
                <label class="text-surface-300 text-sm font-medium">每人金額</label>
...
:deep(.p-inputtext), :deep(.p-dropdown), :deep(.p-inputnumber), :deep(.p-textarea) { background-color: rgba(30, 41, 59, 0.5); border-color: rgba(71, 85, 105, 0.5); }
</style>
