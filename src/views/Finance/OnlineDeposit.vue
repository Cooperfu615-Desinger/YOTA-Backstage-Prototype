<template>
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center gap-2 text-sm text-surface-600 dark:text-surface-400">
      ...
      <span>></span>
      <span class="text-surface-900 dark:text-surface-0 font-medium">在線存款查詢</span>
    </div>
    ...
    <Card class="bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700">
      ...
      <template #content>
        <div class="flex flex-wrap gap-4 mb-4">
          ...
          <div class="flex flex-col gap-1">
            <label class="text-surface-500 dark:text-surface-300 text-sm font-medium">金流商號</label>
    ...
    <Card class="bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700">
      ...
      <template #content>
         <DataTable :value="filteredOrders" :loading="isSearching" stripedRows class="p-datatable-sm" :pt="{ table: { class: 'min-w-full' }, tbody: { class: 'text-surface-700 dark:text-surface-300' } }">
            <template #empty>
            ...
            <Column field="orderId" header="訂單號" style="min-width: 180px">
                <template #body="slotProps">
                    <span class="font-mono text-sm text-blue-500 dark:text-blue-400 cursor-pointer hover:underline" @click="openDetailModal(slotProps.data)">{{ slotProps.data.orderId }}</span>
                </template>
            </Column>
             ...
             <Column field="actualAmount" header="實際入款" style="min-width: 100px">
                 <template #body="slotProps">
                     <div class="flex flex-col">
                        <span class="font-mono font-bold text-surface-900 dark:text-white">{{ formatCurrency(slotProps.data.actualAmount) }}</span>
                        <span class="text-xs text-surface-500 dark:text-surface-400">費率: {{ formatCurrency(slotProps.data.fee) }}</span>
                     </div>
                </template>
            </Column>
            <Column header="支付通道" style="min-width: 160px">
                 <template #body="slotProps">
                    <div class="flex flex-col">
                        <span class="text-surface-900 dark:text-white text-sm">{{ slotProps.data.merchant }}</span>
                        <span class="text-xs text-surface-500 dark:text-surface-400">{{ slotProps.data.type }}</span>
                    </div>
                </template>
            </Column>
            <Column field="applyTime" header="支付時間" style="min-width: 160px">
                 <template #body="slotProps">
                    <span class="text-sm text-surface-500 dark:text-surface-400">{{ slotProps.data.applyTime }}</span>
                </template>
            </Column>
                 ...
                 <template #body="slotProps">
                    <div class="flex items-center gap-2">
                         ...
                         <span v-if="slotProps.data.callbackStatus" 
                            class="w-2 h-2 rounded-full ring-2 ring-offset-1 ring-offset-surface-100 dark:ring-offset-surface-900"
                            :class="slotProps.data.callbackStatus === 'received' ? 'bg-green-500 ring-green-500/30' : 'bg-gray-400 dark:bg-gray-600 ring-gray-400/30 dark:ring-gray-600/30'"
                            v-tooltip.top="'回調狀態'"
                         ></span>
                    </div>
                </template>
    ...
    <Dialog v-model:visible="detailModalVisible" modal :header="`訂單詳情 - ${currentOrder?.orderId}`" :style="{ width: '800px' }">
        <div v-if="currentOrder" class="h-[500px] flex flex-col">
            <div class="flex gap-2 mb-4 border-b border-surface-200 dark:border-surface-700 pb-2">
                 <Button label="日誌追蹤" text :class="activeTab === 'logs' ? 'text-primary' : 'text-surface-500 dark:text-surface-400'" @click="activeTab = 'logs'" />
                 <Button label="原始報文" text :class="activeTab === 'json' ? 'text-primary' : 'text-surface-500 dark:text-surface-400'" @click="activeTab = 'json'" />
            </div>
            ...
            <div class="flex-1 overflow-auto custom-scrollbar">
                ...
                <div v-if="activeTab === 'logs'" class="space-y-4 p-2">
                    <div v-for="(log, idx) in currentOrder.lifecycleLogs" :key="idx" class="flex gap-4 items-start">
                        <div class="w-[150px] text-xs text-surface-500 dark:text-surface-400 font-mono">{{ log.time.replace('T', ' ').substring(0, 19) }}</div>
                        <div class="w-2 h-2 rounded-full bg-primary mt-1.5 shrink-0"></div>
                        <div class="flex-1">
                            <div class="text-sm text-surface-900 dark:text-white font-medium">{{ log.event }}</div>
                            <div class="text-xs" :class="log.status === 'Success' ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'">{{ log.status }}</div>
                        </div>
                    </div>
                </div>
                
                <!-- JSON View -->
                <div v-if="activeTab === 'json'" class="bg-surface-50 dark:bg-black/30 p-4 rounded font-mono text-xs text-surface-700 dark:text-surface-300 overflow-auto">
