    ...
    <Card class="bg-surface-800/50 border border-surface-700">
      ...
      <template #content>
         <DataTable :value="filteredOrders" :loading="isSearching" stripedRows class="p-datatable-sm" :pt="{ table: { class: 'min-w-full' }, tbody: { class: 'text-surface-300' } }">
            ...
            <Column field="type" header="存款類型" style="min-width: 120px">
                 <template #body="slotProps"><span class="text-surface-300">{{ slotProps.data.type }}</span></template>
            </Column>
            ...
            <Column field="applyTime" header="申請時間" sortable style="min-width: 160px">
            ...
            <Column header="操作" style="min-width: 100px">
                <template #body="slotProps">
                     <Button v-if="slotProps.data.status === '待審核'" icon="pi pi-search" severity="info" rounded text v-tooltip.top="'查看詳情'" />
                </template>
            </Column>
         </DataTable>
      </template>
