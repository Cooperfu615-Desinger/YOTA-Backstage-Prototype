<template>
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center gap-2 text-sm text-surface-400">
      ...
      <span>></span>
      <span class="text-white font-medium">人工存提紀錄</span>
    </div>
      ...
      <template #content>
        <div class="flex flex-wrap gap-4 mb-4">
          <div class="flex flex-col gap-1">
           ...
           <div class="flex flex-col gap-1">
            <label class="text-surface-300 text-sm font-medium">操作管理員</label>
            <InputText v-model="filters.operator" placeholder="輸入管理員" class="w-[220px]" />
          </div>
          <div class="flex flex-col gap-1">
            <label class="text-surface-300 text-sm font-medium">操作類型</label>
          ...
          <div class="flex flex-col gap-1">
            <label class="text-surface-300 text-sm font-medium">審核狀態</label>
    ...
    <Card class="bg-surface-800/50 border border-surface-700">
      <template #title>
        <div class="flex items-center gap-2 text-white text-lg">
          <i class="pi pi-pencil text-purple-400"></i>
          人工存提列表
      ...
      <template #content>
         <DataTable :value="filteredLogs" :loading="isSearching" stripedRows class="p-datatable-sm" :pt="{ table: { class: 'min-w-full' }, tbody: { class: 'text-surface-300' } }">
            ...
            <Column field="amount" header="異動金額" style="min-width: 120px">
                 <template #body="slotProps">
                     <span class="font-mono font-bold" :class="slotProps.data.operationType === '增點' ? 'text-green-400' : 'text-red-400'">
            ...
            <Column field="reason" header="操作原因" style="min-width: 200px">
                 <template #body="slotProps">
                    <span class="text-sm text-surface-400 truncate max-w-[180px] block" :title="slotProps.data.reason">{{ slotProps.data.reason }}</span>
                </template>
            </Column>
            ...
            <Column field="approver" header="審核人" style="min-width: 100px">
                 <template #body="slotProps">
                    <span v-if="slotProps.data.approver" class="text-surface-300">{{ slotProps.data.approver }}</span>
