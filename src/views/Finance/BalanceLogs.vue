<template>
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center gap-2 text-sm text-surface-600 dark:text-surface-400">
      ...
      <span>></span>
      <span class="text-surface-900 dark:text-surface-0 font-medium">資金流水紀錄</span>
    </div>
      ...
      <template #content>
        <div class="flex flex-wrap gap-4 mb-4">
          <div class="flex flex-col gap-1">
           ...
           <div class="flex flex-col gap-1">
            <label class="text-surface-500 dark:text-surface-300 text-sm font-medium">關聯單號</label>
            <InputText v-model="filters.orderId" placeholder="輸入關聯單號" class="w-[220px]" />
          </div>
          <div class="flex flex-col gap-1">
            <label class="text-surface-500 dark:text-surface-300 text-sm font-medium">帳務類型</label>
      ...
      <template #content>
         <DataTable :value="filteredLogs" :loading="isSearching" stripedRows class="p-datatable-sm" :pt="{ table: { class: 'min-w-full' }, tbody: { class: 'text-surface-700 dark:text-surface-300' } }">
            ...
            <Column field="beforeAmount" header="異動前金額" style="min-width: 120px">
                 <template #body="slotProps">
                     <span class="font-mono text-surface-500 dark:text-surface-400">{{ formatCurrency(slotProps.data.beforeAmount) }}</span>
                </template>
            </Column>
            <Column field="changeAmount" header="異動金額" style="min-width: 120px">
                 <template #body="slotProps">
                     <span class="font-mono font-bold" :class="slotProps.data.changeAmount >= 0 ? 'text-green-500 dark:text-green-400' : 'text-red-500 dark:text-red-400'">
            ...
            <Column field="afterAmount" header="異動後金額" style="min-width: 120px">
                 <template #body="slotProps">
                     <span class="font-mono text-surface-900 dark:text-white font-bold">{{ formatCurrency(slotProps.data.afterAmount) }}</span>
                </template>
            </Column>
            <Column field="remark" header="備註" style="min-width: 200px">
                 <template #body="slotProps">
                    <span class="text-sm text-surface-500 dark:text-surface-400 truncate max-w-[180px] block" :title="slotProps.data.remark">{{ slotProps.data.remark }}</span>
                </template>
            </Column>
            <Column field="relatedOrder" header="關聯訂單號" style="min-width: 180px">
                 <template #body="slotProps">
                    <span v-if="slotProps.data.relatedOrder" class="font-mono text-sm text-blue-500 dark:text-blue-400">{{ slotProps.data.relatedOrder }}</span>
